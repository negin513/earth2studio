...
class GOES:
    ...
    async def _get_s3_path(self, time: datetime) -> str:
        ...
        prefix = base_url.split(f"{self._bucket}/", 1)[1]

        # List files in the directory to find the closest one. obs.list yields
        # chunks (lists) of entries; rebuild bucket-prefixed paths to match the
        # historical cache-key scheme.
        files = [
            f"{self._bucket}/{entry['path']}"
            for chunk in obs.list(self.store, prefix=prefix)
            for entry in chunk
        ]
